...
def compute_envmap(lgtSGs, H, W, upper_hemi=False):
    # lgtSGs = SGs that represent envmap's lighting

    # exactly same convetion as Mitsuba, check envmap_convention.png

    # created angles depending on whether we are limited to upper hemi or not. Phi angles are spaced by H, 
    # Theta angles are spaced by W
    if upper_hemi:
    ...
    # View directions, using theta and phi, in Spherical Coordinates. needed in SG formula
    ...
    # sanity
    # [..., M, 3]
    # separate lobes(axis), lambdas(sharpness), mus (amplitude)
    # 7 components, first three are lobes, then lambda, and last three are Mus. 
    ...
    # [..., M, 3]
    # Compute formula of SG given axis, viewing direction, sharpness, amplitude
    rgb = lgtSGMus * torch.exp(lgtSGLambdas * (torch.sum(viewdirs * lgtSGLobes, dim=-1, keepdim=True) - 1.))
    rgb = torch.sum(rgb, dim=-2)  # [..., 3] 
    # re-organize SGs in terms of angles of viewing direction
...
#######################################################################################################
#prepend elems of tensor at its beginning in shape "shape"
...
def hemisphere_int(lambda_val, cos_beta):
    #algorithm from book
    #lambda_val is our sharpness, cos_beta is angle we rotate our viewing direction by
...
# convolve two SGs with each other, for example SG that represents cos and SGs that represent light
...
def render_with_sg(lgtSGs, specular_reflectance, roughness, diffuse_albedo, normal, viewdirs, blending_weights=None, diffuse_rgb=None):
    ...
    :return [..., 3]
    '''
    # M = number of Light SGs
    M = lgtSGs.shape[0]
    # K = number of Brdf SGs
    ...
    # expanding = new view of the self tensor with singleton dimensions expanded to a larger size
    ...
    # perform spherical warping (look at spherical warping part in paper)
    ...
    # multiply light sg with warped SGs
    ...
    # final = WITHOUT clamped cos
    # prime = WITH clamped cos
    ...
    if diffuse_rgb is None:
        ...
        final_mus = lgtSGMus.narrow(dim=-2, start=0, length=1) * diffuse # diffuse component here
    ...
    # specular_rgb = specular reflectance, diffuse_rgb = diffuse albedo
    # rgb = mix of specular rgb and diffuse rgb
    # diffuse albedo = proportion of diffuse light reflected off surface 
    ret = {'sg_rgb': rgb,
