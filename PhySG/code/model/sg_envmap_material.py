...
class EnvmapMaterialNetwork(nn.Module):
    def __init__(self, multires=0, dims=[256, 256, 256],
        ...
        self.numBaseMaterials = num_base_materials
        self.numBrdfSGs = 10
        ...
        if self.fix_specular_albedo:
            ...
            if self.white_specular:
                ...
                #encoder/decoder
                self.encoder = nn.Sequential(nn.Linear(self.numBaseMaterials, 8), nn.ReLU(), 
                nn.Linear(8, 4), nn.ReLU(), nn.Linear(4, 1))

                self.decoder = nn.Sequential(nn.Linear(1, 4), nn.ReLU(), 
                nn.Linear(4, 8), nn.ReLU(), nn.Linear(8, self.numBrdfSGs))

                self.specular_reflectance = nn.Parameter(self.decoder(self.encoder(np.transpose(torch.randn(self.numBaseMaterials, 1)))),
    ...
    def get_base_materials(self):
        ...
        if self.fix_specular_albedo:
            ...
            if self.white_specular:
                specular_reflectacne = torch.transpose(specular_reflectacne,0,1).expand((-1, 3))     # [K, 3]
